...
describe('Header -> Custom Component', () => {
  ...
  it('goBackFunction verildiğinde doğru şekilde çalışmalı', async () => {
    jest.clearAllMocks()
    const {component} = testId
    ...
    render(
      <Header
        ...
        back
        goBackFunction={mockGoBack}
      />
    )

    const addListenerMock = mockUseNavigation.mock.results[0].value.addListener.mock
    const beforeRemoveCallback = addListenerMock.lastCall[1]

    beforeRemoveCallback({data: {action: {type: 'GO_BACK'}}})

    expect(mockGoBack).toHaveBeenCalledTimes(1)
  })

  it('goBackFunction verildiğinde preventGoBack ise çalışmamalı', async () => {
    jest.clearAllMocks()
    const {component} = testId
    const mockGoBack = jest.fn()
    ...
    beforeRemoveCallback({data: {action: {type: 'GO_BACK'}}, preventDefault: mockPreventDefault})
