...
export const useDisplay = () => {
  const {referenceDevice} = useContext(RottUiContext)

  // TODO: useDisplay olarak güncellenmeli
  const {height, width} = Dimensions.get('window')

  const widthBaseScale = width / referenceDevice.width
  const heightBaseScale = height / referenceDevice.height
   ...
   */
  const setHeight = (heightPercentage: number) => (referenceDevice.height / 100) * heightPercentage
   ...
   * @returns yuzde girilen ifadeyi **referans cihazin** bilgilerine gore doner
   */
  const setWidth = (widthPercentage: number) => (referenceDevice.width / 100) * widthPercentage
