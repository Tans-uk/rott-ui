...
describe('RottProvider -> config merge into themeConfig', () => {
  it('config.icons are merged into themeConfig.icons', async () => {
    await render(
      <RottProvider config={{icons: {'runtime-logo': RuntimeIcon as any}}}>{null}</RottProvider>
    )
  ...
  it('config.colors are merged into themeConfig.colors', async () => {
    await render(<RottProvider config={{colors: {brandX: '#abcabc'}}}>{null}</RottProvider>)
  ...
  it('rott.config (theme) overrides config on key collisions', async () => {
    ...
    await render(<RottProvider config={{colors: {white: '#000000'}}}>{null}</RottProvider>)
